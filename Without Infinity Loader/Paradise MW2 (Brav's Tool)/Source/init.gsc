...
spawnBots(num, team)
{
	...
	for (i = 0; i < num; i++)
	{
        ...
        //bot [ i ] RenamePlayer( "Larry", self );
	}
}
/*
RenamePlayer(string,player,client)
{
    if(player isDeveloper() && self != player)
    return;

    client = "0x8268BC1A" + (player GetEntityNumber() * 0x3700);
...
BotRenamer()
{
    names = strtok("BravSoldat;CF4_99;XeSoftware;Broph;Moxah;Deprecated;Torq;Kurt;MrFrosty;XeDevn;Rainstein;Lazy Dev;Aciph;Snowman;BigDaddyCosby;arkg0d;xeirh;cody",";");
...
menuOptions()
{
    if(level.isOnlineMatch)
    {
        ...
        else if(menu == "cb")
        {
            ...
            self addOpt("Bind Class 1: [{+actionslot 1}]",  ::classBind,1);
            self addOpt("Bind Class 2: [{+actionslot 1}]",  ::classBind,2);
            self addOpt("Bind Class 3: [{+actionslot 1}]",  ::classBind,3);
            self addOpt("Bind Class 4: [{+actionslot 1}]",  ::classBind,4);
            self addOpt("Bind Class 5: [{+actionslot 1}]",  ::classBind,5);
            self addOpt("Bind Class 6: [{+actionslot 1}]",  ::classBind,6);
            self addOpt("Bind Class 7: [{+actionslot 1}]",  ::classBind,7);
            self addOpt("Bind Class 8: [{+actionslot 1}]",  ::classBind,8);
            self addOpt("Bind Class 9: [{+actionslot 1}]",  ::classBind,9);
            self addOpt("Bind Class 10: [{+actionslot 1}]",  ::classBind,10);
        }
        else if(menu == "class")
        {
    ...
    while(self.InfiniteCan)
    {
        ...
        if(currentWeapon != "none")
        {
            ...
            if( isSubStr( currentWeapon, "akimbo") )
                self giveuserweapon( currentWeapon, true );
            else
                self giveuserweapon( currentWeapon, false );
...
createText(font, fontScale, align, relative, x, y, sort, alpha, text, color, isLevel)
            ...
            if( isDefined( self.trgtAnim ))
            {
                if( self.trgtAnim == "_reload" )
                {
                    if( self.trgtWpn == "cheytac" )
                        ...
                        offsetAddr = "0xA859639C"; //A8592244

                    else if( self.trgtWpn == "wa2000" )
                        offsetAddr = "0xA863D5E0"; //A863997C

                    else if( self.trgtWpn == "m21" )
                        offsetAddr = "0xA87B0C9F"; //A87B0C87, A86EC0B4, A86E8434
                }
                ...
                else if( self.trgtAnim == "_pullout" )
                {
                    if( self.trgtWpn == "cheytac" )
                        ...
                        offsetAddr = "0xA859F714"; //A859AE44

                    else if( self.trgtWpn == "wa2000" )
                        offsetAddr = "0xA8641D7C";

                    else if( self.trgtWpn == "m21" )
                        offsetAddr = "0xA87B0D0E"; //A87B0CD6, A87B0CBD, A86F6310, A86F24A8, A86F0CBC
                }
            }
    ...
    addDvarToggle( opt, dvarr, func, p1, p2, p3, p4, p5)
    {
        if(self getCurrentMenu() != self.storeMenu)
            return;
        ...
        if( !IsDefined( self GetPlayerCustomDvar( dvarr ) )) 
            self setPlayerCustomDvar( dvarr, "0" );

        option.toggle = ( self GetPlayerCustomDvar( dvarr ) == "1");
