    ...
    setMenuText()
    {
        ...
        for(e=0;e<10;e++)
        {
            ...
            if(IsDefined( self.eMenu[ (ary + e) ].ID_list ) )
            {
                ...
                #else
                self.menu["UI_SLIDE"]["STRING_"+e] = self createText("objective", 0.8, "RIGHT", "CENTER", self.menu["OPT"][e].x + 193, self.menu["OPT"][e].y, 6, 1, "", self.presets["Text"]);
                #endif
                #else
            ...
            if(self.eMenu[ ary + e ].func == ::newMenu && IsDefined( self.eMenu[ ary + e ].func ) )
            {
                #ifdef BO3 || IW
                #ifdef IW
                self.menu["UI_SLIDE"]["SUBMENU"+e] = self createtext("objective", 0.8, "RIGHT", "CENTER", self.menu["OPT"][e].x + 196, self.menu["OPT"][e].y - 0.75, 5, 1, ">", (1,1,1));
                #else
