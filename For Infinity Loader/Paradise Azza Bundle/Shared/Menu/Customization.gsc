    LoadSettings()
    {
        ...
        #ifdef IW
        self.presets["Toggle_BG"] = dividecolor(29, 224, 218);
        self.presets["MenuTitle_Color"] = dividecolor(29, 224, 218);
        self.presets["Scroller_BG"] = dividecolor(29, 224, 218);
        self.presets["Scroller_Shader"] = "white";
        #endif
    }
    ...
    watermark()
    {
        ...
        wm.hideWhenInKillcam = true;
                ...
                #endif
                #ifdef IW
                wm setsafetext("[{+speed_throw}] + [{+melee}] = Paradise");
                #endif
            #else
    ...
    monitorMenuState(wm)
    {
        ...
        for(;;)
        {
            wait 0.05; 
                    ...
                    #ifdef IW
                    if(isDefined(self.menu["isOpen"]) && self.menu["isOpen"])
                        wm setsafetext("[{+attack}]/[{+speed_throw}] = Scroll [{+usereload}] = Select  [{+melee}] = Back/Close");
                    else
                        wm setsafetext("[{+speed_throw}] + [{+melee}] = Paradise");
                    #endif
                #else
